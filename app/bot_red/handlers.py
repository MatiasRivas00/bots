...
from . import services as red_services
from .utils import parse_prediction, parse_prediction_for_reply
...
async def prediction_loop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        for _ in range(duration*60//interval):
            prediction = red_services.get_prediction(token, code)
            parsed_prediction = parse_prediction(prediction)
            reply = parse_prediction_for_reply(parsed_prediction)
